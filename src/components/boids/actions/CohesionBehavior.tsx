...
export class CohesionBehavior {
  private boid: Boid; // The boid for which the cohesion behavior is calculated
  ...
  calculate(): p5.Vector {
    ...
    for (let other of this.boids) {
      ...
      if (d > 0 && d < perceptionRadius) { // Check if the other boid is within the perception radius
        steering.add(other.position); // Add the other boid's position to the steering force
        total++; // Increment the total number of boids within the perception radius
      }
    }

    if (total > 0) {
      // Calculate the average position of the boids within the perception radius
      steering.div(total);
      steering.sub(this.boid.position); // Subtract the boid's current position from the average position
